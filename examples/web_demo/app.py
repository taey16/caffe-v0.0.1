...
class ImagenetClassifier(object):
  ...
  def enroll_image(self, image, filename):
    try:
      ...
      filename = '/enroll/' + filename
      self.database['ref'] = np.vstack((self.database['ref'], fea))
      self.database['dic_ref'][self.database['ref'].shape[0]-1] = filename
      self.database['dic_idx'][filename] = self.database['ref'].shape[0]-1
      logging.info('dic_ref[{}] = {}'.format(self.database['ref'].shape[0], self.database['dic_ref'][self.database['ref'].shape[0]]))
  ...
  def classify_image(self, image):
    try:
      ...
      scores_google = self.net_google.predict([image], oversample=True).flatten()
      endtime = time.time()
      logging.info('Predict done for %d classes', scores.shape[0])
...
def start_from_terminal(app):
  ...
  if opts.debug:
    app.run(debug=True, host='10.202.211.120', port=opts.port)
  else:
