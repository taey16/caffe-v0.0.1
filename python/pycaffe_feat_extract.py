...
if __name__ == '__main__':
    PATH_MODEL_DEF_FILE = '/works/neuraltalk/python_features/deploy_features.prototxt'
    PATH_MODEL = '/works/caffe/models/vgg/vgg_layer16.caffemodel'
    WITH_GPU = 1
    #start_time = time.time()
    net = caffe_load_model(PATH_MODEL_DEF_FILE, PATH_MODEL, WITH_GPU)

    INPUT_LIST = '../data/ilsvrc12/test.txt'
    INPUT_ROOT = '/storage/ImageNet/ILSVRC2012/test/'
    path_imgs = ['%s%s' % (INPUT_ROOT, entry.strip().split(' ')[0]) for entry in open(INPUT_LIST)]
    path_imgs = path_imgs[:10]
